...
describe('BankAccount', () => {
  ...
  beforeEach(
    () => (bankAccountInstance = getBankAccount(TEST_INITIAL_BALANCE)),
  );
  ...
  test('should throw InsufficientFundsError error when withdrawing more than balance', () => {
    expect(() => bankAccountInstance.withdraw(800)).toThrowError(
      InsufficientFundsError,
    );
  });
  ...
  test('should throw error when transferring more than balance', () => {
    expect(() =>
      bankAccountInstance.transfer(800, getBankAccount(TEST_INITIAL_BALANCE)),
    ).toThrowError();
  });

  test('should throw error when transferring to the same account', () => {
    expect(() =>
      bankAccountInstance.transfer(800, bankAccountInstance),
    ).toThrowError();
  });
  ...
  test('should throw SynchronizationFailedError if fetchBalance returned null', async () => {
    ...
    expect(() => bankAccountInstance.synchronizeBalance()).rejects.toThrowError(
      SynchronizationFailedError,
    );
  });
});
